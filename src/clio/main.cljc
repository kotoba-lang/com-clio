(ns clio.main
  "Kotodama WASM entrypoint for the Clio clean-room actor (L4) — Clojure port.
  ...
  facts (`clio.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Record"   :plural "records"   :id-prefix "clio_rec"
    :fields [:name :type :ownerId :status]        :required [:name :type]
    :coerce {}                       :refs {}}
   {:entity "Matter"   :plural "matters"   :id-prefix "clio_mat"
    :fields [:clientId :title :status :openedAt]  :required [:title :status]
    :coerce {}                       :refs {}}
   {:entity "Contract" :plural "contracts" :id-prefix "clio_con"
    :fields [:counterparty :value :status :signedAt] :required [:counterparty :value]
    :coerce {:value :float}          :refs {}}
   {:entity "Course"   :plural "courses"   :id-prefix "clio_cou"
    :fields [:title :term :enrollment]             :required [:title :term]
    :coerce {:enrollment :int}       :refs {}}
   {:entity "Property" :plural "properties" :id-prefix "clio_pro"
    :fields [:address :units :value]               :required [:address :units]
    :coerce {:units :int :value :float} :refs {}}
   {:entity "Document" :plural "documents" :id-prefix "clio_doc"
    :fields [:recordId :name :contentRef]          :required [:name]
    :coerce {}                       :refs {:recordId "Record"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"clio.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "clio-compat" :tier tier :entities entities} 200])
